...
exports.handleDriveNotification = functions.https.onRequest(
  async (req, res) => {
    ...
    // Check if the request has a valid X-Goog-Channel-Token header
    // If DEFINED, it must be equal to the channel token
    // const channelToken = req.get("X-Goog-Channel-Token");
    // if (!channelToken) {
    //   // Return a 400 Bad Request response
    //   functions.logger.error("Missing X-Goog-Channel-Token header");
    //   res.status(400).send("Missing X-Goog-Channel-Token header");
    //   return;
    // }
    ...
    // Get the channel document by channelId
    const channelDoc = await db.collection("channels").doc(channelId).get();

    // If the channel document does not exist, return a 404 Not Found response
    if (!channelDoc.exists) {
      functions.logger.error("Channel document does not exist");
      res.status(404).send("Channel document does not exist");
      return;
    }
    // Otherwise log the channel document data
    functions.logger.info("Channel document data:");
    functions.logger.info(channelDoc.data());

    // Get the user uid
    const uid = channelDoc.data().uid;

    // Get the folder id
    const folderId = channelDoc.data().fileId;

    // Get the last notification timestamp
    const lastNotification = channelDoc
      .data()
      .lastNotification.toDate()
      .toISOString();

    functions.logger.info(`Got user and folder:`);
    functions.logger.info(`User ID: ${uid}`);
    functions.logger.info(`Folder ID: ${folderId}`);
    functions.logger.info(`Last notification: ${lastNotification}`);

    // Get the user document by uid
    functions.logger.info(`Last notification timestamp: ${lastNotification}`);
...
exports.transcribeAudio = functions
  ...
  .onFinalize(async (object) => {
    ...
    const transcription = await whisperTranscribe(bytes[0], process.env.OPENAI_API_KEY);
...
async function whisperTranscribe(audioBytes, openaiApiKey) {
  ...
  if (openaiApiKey === null) {
    functions.logger.info("openai api key not passed to whisperTranscribe. Attepmting to get openai api key from env");
